...
public class StubContentProvider extends ContentProvider {
	...
	public Bundle call(String method, String arg, Bundle extras) {
		if (VConstant.VA_INIT_PROCESS_KEY.equals(method)) {
	...
	private Bundle initProcess(Bundle extras) {
		...
		IBinder token = BundleCompat.getBinder(extras, VConstant.VA_BINDER_KEY);
		int vuid = extras.getInt(VConstant.VA_VUID_KEY);
		VClientImpl client = VClientImpl.get();
		client.initProcess(token, vuid);
		Bundle res = new Bundle();
		BundleCompat.putBinder(res, VConstant.VA_CLIENT_KEY, client.asBinder());
		res.putInt(VConstant.VA_PID_KEY, Process.myPid());
		return res;
	}
	@Override
