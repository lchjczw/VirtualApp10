...
import static android.content.pm.ActivityInfo.LAUNCH_SINGLE_TOP;
 class ActivityStack {
    ...
    int startActivityLocked(int userId, Intent intent, ActivityInfo info, IBinder resultTo, Bundle options,
        ...
        VLog.e("从 VActivityManager.startActivity 走到了 ActivityStack ,其中 userId = "+userId+"; intent is null ? "+( intent == null)+"; resultTo is null ? "+(resultTo == null) +"; resultWho is null ? "+(resultWho == null)+"; options is null ? "+( options == null) +"; requestCode is "+requestCode);
        optimizeTasksLocked();
        Intent destIntent;
        ActivityRecord sourceRecord = findActivityByToken(userId, resultTo);
        VLog.d("ActivityStack startActivityLocked 第一次 来应该是 空？ "+(sourceRecord == null));
        VLog.d("ActivityStack sourceRecord "+sourceRecord);
        ...
        VLog.d("reuseTask is null ? "+(reuseTask == null));
        if (reuseTask == null) {
            ...
            if (!startTaskToFront) {
                if (!delivered) {
                    ...
                    if (destIntent != null) {
                        VLog.d("这里将走到startActivityFromSourceTask");
    ...
    private void startActivityInNewTaskLocked(int userId, Intent intent, ActivityInfo info, Bundle options) {
        ...
        if (destIntent != null) {
            ...
            VLog.i("  OK 终于调用到了 startActivity 向 VA 里面的 IActivityManager 请求, 这个请求 将被 代理者 ActivityManagerStub 实现截获 打开 Host Stub Activity destIntent:"+destIntent.toString());
    ...
    private void realStartActivitiesLocked(IBinder resultTo, Intent[] intents, String[] resolvedTypes, Bundle options) {
        ...
        if (IActivityManager.startActivities != null) {
            types = IActivityManager.startActivities.paramList();
        }
    ...
    private String fetchStubActivity(int vpid, ActivityInfo targetInfo) {
        ...
        VLog.d("根据在 Menifest 中注册的 vpid = "+vpid+"; 返回 StubActivity 全限定名 ");
        if (isDialogStyle) {
    ...
    private Intent startActivityProcess(int userId, ActivityRecord sourceRecord, Intent intent, ActivityInfo info) {
        ...
        VLog.d("启动一个Acitivity进程 当前进入参数 userId："+userId+"    sourceRecord:"+sourceRecord+"    intent:"+intent+"    info:"+info);
        ProcessRecord targetApp = mService.startProcessIfNeedLocked(info.processName, userId, info.packageName);
        if (targetApp == null) {
            return null;
        }
        Intent targetIntent = new Intent();
        DebugKook.i("根据 Client App 的 PID 获取 StubActivity");
        String fetchStubActivity = fetchStubActivity(targetApp.vpid, info);
        VLog.d("startActivityProcess fetchStubActivity:"+fetchStubActivity);
        targetIntent.setClassName(VirtualCore.get().getHostPkg(),fetchStubActivity);
    ...
    void onActivityCreated(ProcessRecord targetApp, ComponentName component, ComponentName caller, IBinder token,
        ...
        synchronized (mHistory) {
            ...
            DebugKook.i(" 创建完 ActivityRecord "+record);
            synchronized (task.activities) {
