...
public class VPackageManagerService implements IPackageManager {
    ...
    public ProviderInfo resolveContentProvider(String name, int flags, int userId) {
        VLog.e(" VPackageManagerService  resolveContentProvider name;"+name+"   flags:"+flags+"    userId:"+userId);
        checkUserId(userId);
        flags = updateFlagsNought(flags);
        synchronized (mPackages) {
            final VPackage.ProviderComponent provider = mProvidersByAuthority.get(name);
            VLog.e(" VPackageManagerService  resolveContentProvider provider:"+(provider != null)+"    mProvidersByAuthority:"+mProvidersByAuthority.size()+"    name:"+name);
            if (provider != null) {
                PackageSetting ps = (PackageSetting) provider.owner.mExtras;
                ProviderInfo providerInfo = PackageParserEx.generateProviderInfo(provider, flags, ps.readUserState(userId), userId);
                VLog.e(" VPackageManagerService  resolveContentProvider providerInfo:"+(providerInfo));
