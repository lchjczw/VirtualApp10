...
public class PackageParserEx {
    ...
    public static void initApplicationInfoBase(PackageSetting ps, VPackage p) {
        ...
        VLog.i(" initApplicationInfoBase ai.nativeLibraryDir:"+ai.nativeLibraryDir);
        ai.uid = ps.appId;
    ...
    private static void initApplicationAsUser(ApplicationInfo ai, int userId) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            ...
            if (Build.VERSION.SDK_INT > AndroidVersionConstant.Oreo_8) {
                ApplicationInfoP.deviceProtectedDataDir.set(ai, ai.dataDir);
                ApplicationInfoP.credentialProtectedDataDir.set(ai, ai.dataDir);
            }else {
                VLog.e("这里没有deviceProtectedDataDir");
            }
        }
    }
    ...
    public static ApplicationInfo generateApplicationInfo(VPackage p, int flags,  PackageUserState state, int userId) {
        if (p == null){
            return null;
        }
        if (!checkUseInstalledOrHidden(state, flags)) {
