...
public class IApplicationThreadCompat {
    public static void scheduleCreateService(IInterface appThread, IBinder token, ServiceInfo info,  int processState) throws RemoteException {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            IApplicationThread.scheduleCreateService.call(appThread, token, info, CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO.get(), processState);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            IApplicationThread.scheduleCreateService.call(appThread, token, info, CompatibilityInfo.DEFAULT_COMPATIBILITY_INFO.get());
        } else {
    ...
    public static void scheduleServiceArgs(IInterface appThread, IBinder token, boolean taskRemoved,
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            ...
            IApplicationThread.scheduleServiceArgs.call(appThread, token, ParceledListSliceCompat.create(list));
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH_MR1) {
            IApplicationThread.scheduleServiceArgs.call(appThread, token, taskRemoved, startId, flags, args);
        } else {
